...
const getAllMobilesController = async (req, res) => {
  try {
    ...
    const mobiles = await mobileModel.find({ sold: false });//{ sellerId: { $ne: userId } }
...
const getUserAddedMobilesController = async (req, res) => {
  try {
    ...
    const mobiles = await mobileModel.find({ sellerId: userId, sold: false });
    res.status(200).json({ success: true, mobiles });
  } catch (error) {
    console.error('Error fetching user mobiles:', error);
    res.status(500).json({ success: false, message: 'Internal Server Error' });
...
const getLatestMobile = async (req, res) => {
  try {
    ...
    const latestMobile = await mobileModel.find({ sold: false })//{ sellerId:  {$ne: userId } }
